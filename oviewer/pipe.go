...
func (root *Root) pipeBuffer(input string) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "failed to create document: %s\n", err)
		return
	}
	docout.FileName = fmt.Sprintf("[Pipe to: %s]", cmdStr)

	if err := root.runPipeCommand(docout, cmdStr); err != nil {
...
func (root *Root) runPipeCommand(docout *Document, cmdStr string) error {
	...
	docout.ControlReader(so, nil)
	cmd.Stderr = os.Stderr
	root.AddDocument(docout)
